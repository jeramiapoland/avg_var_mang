...
\begin{frame}{Central Idea I}
	\begin{itemize}[<+->]
		...
		\item $SV_{t+1} = \alpha_{3} + \beta_{3}AV_{t} + \epsilon_{3,t}$, $\beta_{3}$ positive and significant
		\item AV$_{t}$ management decreases investment when AV$_{t}$ is high (future risk will be high, returns not coming)
...
\begin{frame}{Central Idea II}
	\begin{itemize}[<+->]
		...
		\item If it is correct, when AC is high and higher $r_{s,t+1}$ is expected $\Rightarrow$ higher $r_{u,t+1}$
...
\begin{frame}{Investor Utility}
	\only<1>{
	\begin{figure}
		\resizebox{11cm}{6cm}{\input{figures/cer_const_plot}}
	\end{figure}
	}
	\only<2-3>{
		\begin{figure}
			\resizebox{11cm}{6cm}{\input{figures/cer_gamma_plot}}
		\end{figure}
	}
