...
\begin{frame}{Background}
	\begin{itemize}[<+->]
		...
		\item Risk != Reward : Haugen and Heins (1972), Campbell (1987), French et al. (1987), Golsten et al. (1993)
		\item Moreira and Muir (2017) - scaling month t+1 investment in the market by month t daily return variance
...
\begin{frame}{Central Idea I}
	\begin{itemize}[<+->]
		...
		\item Moreira and Muir (2017) : weight = $\frac{c_{SV}}{SV_{t}}$, c is scaling factor used to match variance to buy and hold for comparison
		\item Me : weight = $\frac{c_{AV}}{AV_{t}}$
		\item AV$_{t}$ management decreases investment when AV$_{t}$ is high (future risk will be high)
		\item AV$_{t}$ management avoids decreasing investment when SV$_{t}$ is high because AC is high (future returns are coming)
	\end{itemize}
\end{frame}
...
\begin{frame}{More Pollet and Wilson (2010)}
	\begin{itemize}[<+->]
		...
		\item Shocks to true market returns are positively related to the common component of observed stock market variance, AC
		\item AC is positively related to the covariance of stock market returns and aggregate wealth
		\item AV is negatively related to the covariance of stock market returns and aggregate wealth
		\item The relationships depend on the proportion of the true market observed, $w_{s,t}$, and the aggregate wealth $\beta_{t}$ for stock market returns 
...
\begin{frame}{Central Idea II}
	\begin{itemize}[<+->]
		...
		\item If it is correct, when AC is high and higher $r_{s,t+1}$ is expected higher $r_{u,t+1}$
		\item This holds so long as AC (AV) is calculated from daily returns which are sufficiently representative of all asset returns and asset returns are significantly related to returns no aggregate wealth
		\item Suggestive evidence is support of Pollet and Wilson (2010) relation
		\begin{itemize}[<+->]
			\item In a subset where daily returns are unrepresentative, AC won't predict
			\item AV management has higher investments in time of higher AC relative to SV management - this should work globally
			\item AV management has higher investment in times of higher $cov(r_{s,t+1},r_{u,t+1})$, so it should work across asset classes
		\end{itemize}
	\end{itemize}
\end{frame}
...
%	\only<1>{\begin{figure}
%		\resizebox{11cm}{6.5cm}{\input{figures/m_ret_plot2_1}}
%	\end{figure}
%	\centering
%	AV : 7.89\%  SV : 6.17\%  BH : 5.93\%} 
%	\only<2>{\begin{figure}
%		\resizebox{11cm}{6.5cm}{\input{figures/m_ret_plot2_2}}
%	\end{figure}
%	\centering
%	AV : 9.68\%  SV : 7.61\%  BH : 5.93\% }
%	\only<3>{\begin{figure}
%			\resizebox{11cm}{6.5cm}{\input{figures/m_ret_plot2_3}}
%		\end{figure}
%		\centering
...
\begin{frame}{Equity Performance}
...
\begin{frame}{Not US Equity Performance}
...
\begin{frame}{Contribution}
	\onslide<+->{\begin{block}{Variance Management}
			\begin{itemize}[<+->]
				...
				\item AV management works globally and for a globally diversified equity portfolio
				\item AV management works across asset class 
			\end{itemize}
		\end{block}}
		\onslide<+->{\begin{block}{Risk Dynamics}
				\begin{itemize}[<+->]
					\item AV comes from the foundations of investment risk
					\item AV management is better because it responds not just to total risk, but the mix of systematic and non-systematic
...
\begin{frame}{Investment Weights}
			\begin{figure}
			\scalebox{.52}{\input{figures/weight_plot2}}
			\end{figure}
\end{frame}
\begin{frame}{Investment Weights Again}
	\begin{figure}
		\scalebox{.65}{\input{figures/weight_plot3}}
	\end{figure}
\end{frame}
...
\begin{frame}{Return Prediction}
	...
	\begin{table}
		\begin{adjustbox}{width=\textwidth,height=3.5cm}
			\input{tables/insample/tab_in_logxret.tp1_post1962}
		\end{adjustbox}
	\end{table}
\end{frame}
...
%		\vspace{-6pt}
%		\begin{adjustbox}{center,max height=\totalheight}
%			\input{tables/outsample/tab_oos_avg_var1m_pre1962}
%		\end{adjustbox}
%	\end{table}
%\end{frame}
